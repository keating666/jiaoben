...
```
jiaoben/
├── src/              # Cloudflare Worker 源代码（生产环境）
├── apps/             # 前端应用 (Turborepo)
│   └── creator-app/  # C端应用 (Next.js)
├── tech-validation/  # 技术验证代码和测试
├── archive/          # 已废弃的代码归档
...
## 当前技术栈说明 (2025-12-24 更新)

### 🎯 统一环境：Cloudflare Worker
**生产和开发环境统一使用 Cloudflare**
1. **TikHub** - 获取视频真实链接
...
### 📁 本地开发
```bash
wrangler dev    # 本地运行 Cloudflare Worker
```

### ⚠️ 已废弃的服务（代码已归档）
- **YunmaoClient** - 云猫转码（已移至 `archive/deprecated-services/`）
- **IflyTek** - 科大讯飞（已移至 `archive/deprecated-services/`）
- **Vercel API** - 原 Vercel Serverless Functions（已移至 `archive/vercel-api-deprecated/`）
...
2. **已实现的功能**
   ...
   - Story 0.2: 视频转文字编排服务（95.25% DOD 合规）
     - 视频下载和音频提取
     - **统一环境**：Cloudflare Worker + 腾讯云 ASR
     - Tongyi 脚本生成集成
     - 完整的错误处理
