/**
 * Cloudflare Worker - 简化版通义千问方案
 * 暂时跳过ASR，直接使用模拟数据测试通义千问
 */
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // 简化版处理流程
      if (url.pathname === '/api/process-simple') {
        console.log('收到简化处理请求');
        const body = await request.json();
        ...
        // 步骤3：生成模拟转写内容（基于视频标题）
        const mockTranscript = generateSmartTranscript(aweme?.desc || '抖音视频');
        
        // 步骤4：使用通义千问生成分镜脚本
        console.log('使用通义千问生成脚本...');
