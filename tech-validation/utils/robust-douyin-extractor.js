...
exports.RobustDouyinExtractor = void 0;
const logger_1 = require("./logger");
const regex_safety_1 = require("./regex-safety");
const enhanced_douyin_extractor_1 = require("./enhanced-douyin-extractor");
/**
 * 健壮版抖音链接提取器
 * 增强了安全性、性能和错误处理
 */
class RobustDouyinExtractor extends enhanced_douyin_extractor_1.EnhancedDouyinExtractor {
    /**
     * 安全的智能提取（主入口）
     */
    static async smartExtract(text) {
        try {
            ...
            logger_1.logger.error('RobustDouyinExtractor', 'smartExtract', '提取失败', error);
            // 返回安全的默认结果
            return {
                links: [],
                commands: [],
     ...
     * 分块处理长文本
     */
    static async extractInChunks(text) {
        const chunks = [];
        ...
        // 并行处理各个块
        const chunkResults = await Promise.all(chunks.map(chunk => this.robustExtract(chunk)));
        // 合并结果
     ...
     * 健壮的提取实现
     */
    static async robustExtract(text) {
        const links = [];
        const commands = [];
        const processedUrls = new Set();
        const processedCommands = new Set();
        // 1. 提取链接（使用安全执行器）
        const linkResults = regex_safety_1.RegexSafety.safeExecMultiple(this.ROBUST_URL_PATTERNS, text, {
            timeout: 100,
        ...
        // 2. 提取口令（使用安全执行器）
        const commandResults = regex_safety_1.RegexSafety.safeExecMultiple(this.ROBUST_COMMAND_PATTERNS, text, {
            timeout: 100,
        ...
        for (const [pattern, matches] of commandResults) {
            for (const match of matches) {
                if (commands.length >= this.MAX_COMMANDS)
                    break;
     ...
     * 健壮的URL清理
     */
    static robustCleanUrl(url, fullText, position) {
        try {
            ...
            try {
                ...
                const isValid = validDomains.some(domain => hostname.endsWith(domain) || hostname.endsWith(`.${domain}`));
                if (!isValid) {
                    return null;
                }
                return cleaned;
            ...
            logger_1.logger.warn('RobustDouyinExtractor', 'robustCleanUrl', 'URL清理失败', { url });
            return null;
        }
    }
    /**
...
exports.RobustDouyinExtractor = RobustDouyinExtractor;
// 输入限制
RobustDouyinExtractor.MAX_TEXT_LENGTH = 50000;
RobustDouyinExtractor.MAX_LINKS = 100;
RobustDouyinExtractor.MAX_COMMANDS = 100;
RobustDouyinExtractor.CHUNK_SIZE = 5000;
// 优化后的URL模式（添加了长度限制）
RobustDouyinExtractor.ROBUST_URL_PATTERNS = [
    // 短链接（限制ID长度）
...
// 优化后的口令模式（添加了边界检查）
RobustDouyinExtractor.ROBUST_COMMAND_PATTERNS = [
    // 话题标签（限制长度）
    /[#＃]{1}[^#＃]{1,100}[#＃]{1}/g,
    // 数字代码格式（更严格的匹配）
