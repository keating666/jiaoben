...
class RegexSafety {
    ...
    static validatePattern(pattern) {
        ...
        const dangerousPatterns = [
            ...
            { pattern: /\+\+/, reason: '包含占有量词' },
            { pattern: /\{(\d+),\}/g, reason: '包含大量重复', check: (match) => {
                    const min = parseInt(match[1]);
                    return min > 100;
                } },
            { pattern: /\{(\d+),(\d+)\}/g, reason: '包含大量重复', check: (match) => {
     ...
     * 安全执行正则表达式
     */
    static safeExec(pattern, text, options = {}) {
     ...
     * 批量安全执行多个正则
     */
    static safeExecMultiple(patterns, text, options = {}) {
     ...
     * 测试正则性能
     */
    static benchmarkPattern(pattern, testTexts) {
        const times = [];
        let safe = true;
...
exports.RegexSafety = RegexSafety;
RegexSafety.MAX_EXECUTION_TIME = 1000; // 最大执行时间（毫秒）
RegexSafety.MAX_ITERATIONS = 10000; // 最大迭代次数
