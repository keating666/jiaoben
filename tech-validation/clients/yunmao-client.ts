...
interface ExtractTextRequest {
  ...
  language?: string;         // 语言设置 (zh-CN, en-US, ja-JP, ko-KR等)
  dialogueMode?: boolean;    // 是否开启对话模式
  speakerCount?: number;     // 说话人数量（对话模式时使用）
  outputFormat?: 'text' | 'txt' | 'srt' | 'vtt'; // 输出格式
 ...
 * 任务状态查询响应
 */
interface TaskStatusResponse extends ExtractTextResponse {
  createdAt: string;
  updatedAt: string;
...
export class YunmaoClient {
  ...
  constructor(config: YunmaoConfig) {
    // 设置默认值
    this.config = {
   ...
   * 创建视频转文字任务
   */
  async createExtractTextTask(params: ExtractTextRequest): Promise<ExtractTextResponse> {
    logger.info('YunmaoClient', 'createExtractTextTask', '创建视频转文字任务', {
    ...
    try {
      ...
      const response = await this.client.post<any>(
        `${this.config.baseUrl}/extract-text`,
        requestData,
        {
          headers: {
      ...
      logger.error('YunmaoClient', 'createExtractTextTask', '创建任务失败', error as Error);
  ...
  async getTaskStatus(taskId: string): Promise<TaskStatusResponse> {
    logger.info('YunmaoClient', 'getTaskStatus', '查询任务状态', { taskId });
  ...
  async waitForCompletion(
    ...
    logger.info('YunmaoClient', 'waitForCompletion', '开始等待任务完成', { taskId });
    ...
    while (true) {
      ...
      if (status.status === 'completed') {
        logger.info('YunmaoClient', 'waitForCompletion', '任务完成', { taskId });
        return status;
      }
   ...
   * 解析视频转文字响应
   */
  private parseExtractTextResponse(data: any): ExtractTextResponse {
    return {
   ...
   * 解析任务状态响应
   */
  private parseTaskStatusResponse(data: any): TaskStatusResponse {
  ...
  private handleError(error: any): Error {
    if (error.response) {
      ...
      switch (status) {
        ...
        case 500:
          return new Error(`服务器错误: ${data?.message || '请稍后重试'}`);
        default:
          return new Error(`API错误 (${status}): ${data?.message || error.message}`);
      }
    }
