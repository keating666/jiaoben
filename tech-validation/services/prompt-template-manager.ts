...
export interface PromptTemplate {
  ...
  version: string;
  template: string;
  variables: string[];  // 模板中的变量列表
  category: 'script' | 'summary' | 'analysis' | 'custom';
  isActive: boolean;
  createdAt: string;
  updatedAt: string;
  author?: string;
...
export class PromptTemplateManager {
  ...
  private readonly defaultTemplates: PromptTemplate[];

  constructor(templatesDir?: string) {
    this.templatesDir = templatesDir || path.join(process.cwd(), 'config', 'prompts');
...
请输出专业级的分镜头脚本（JSON格式）：`,
  ...
  async initialize(): Promise<void> {
    logger.info('PromptTemplateManager', 'initialize', '初始化模板管理器');
    ...
    try {
      ...
      logger.warn('PromptTemplateManager', 'initialize', '加载自定义模板失败，使用默认模板', {
        error: error instanceof Error ? error.message : String(error)
      });
    }

    logger.info('PromptTemplateManager', 'initialize', `加载了 ${this.templates.size} 个模板`);
  }
      ...
      for (const file of jsonFiles) {
        try {
          ...
          if (this.validateTemplate(template)) {
            this.templates.set(template.id, template);
            logger.info('PromptTemplateManager', 'loadTemplatesFromDisk', `加载模板: ${template.name}`);
          }
        } catch (error) {
          logger.error('PromptTemplateManager', 'loadTemplatesFromDisk', `加载模板文件失败: ${file}`, error as Error);
        }
      }
    } catch (error) {
      logger.error('PromptTemplateManager', 'loadTemplatesFromDisk', '读取模板目录失败', error as Error);
    }
  }
   ...
   * 验证模板格式
   */
  private validateTemplate(template: any): template is PromptTemplate {
    return (
      template &&
   ...
   * 获取所有激活的模板
   */
  getActiveTemplates(category?: string): PromptTemplate[] {
   ...
   * 渲染模板
   */
  renderTemplate(templateId: string, params: PromptRenderParams): string {
    ...
    const variables: Record<string, any> = {
      ...
      videoTitle: params.videoTitle || '未命名视频',
      videoDuration: params.videoDuration || 0,
      videoAuthor: params.videoAuthor || '未知作者',
    ...
    if (unreplaced) {
      logger.warn('PromptTemplateManager', 'renderTemplate', '存在未替换的变量', {
        templateId,
        unreplaced
      });
    }
  ...
  async saveTemplate(template: PromptTemplate): Promise<void> {
    ...
    try {
      ...
      logger.info('PromptTemplateManager', 'saveTemplate', `模板已保存: ${template.name}`);
    } catch (error) {
      logger.error('PromptTemplateManager', 'saveTemplate', '保存模板失败', error as Error);
      throw error;
    }
  }
  ...
  async deleteTemplate(templateId: string): Promise<void> {
    ...
    try {
      ...
      logger.info('PromptTemplateManager', 'deleteTemplate', `模板已删除: ${templateId}`);
    } catch (error) {
      logger.warn('PromptTemplateManager', 'deleteTemplate', '删除模板文件失败', {
        templateId,
        error: error instanceof Error ? error.message : String(error)
      });
    }
