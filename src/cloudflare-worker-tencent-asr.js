...
async function generateScriptWithQwen(videoInfo, env) {
  // 构建个性化提示词
  let prompt = `请根据以下信息，创作一个专业的5-8个场景分镜脚本：
...
`;
  ...
  // 添加个性化信息
  const p = videoInfo.personalization || {};
  if (p.nickname || p.industry || p.purpose || p.style) {
    prompt += `
【个性化要求】`;
    if (p.nickname) prompt += `
创作者：${p.nickname}`;
    if (p.industry) prompt += `
所属行业：${p.industry}`;
    if (p.purpose) prompt += `
商业目的：${p.purpose}`;
    if (p.targetAge) prompt += `
目标受众：${p.targetAge}`;
    if (p.targetGender && p.targetGender !== '不限') prompt += `
性别倾向：${p.targetGender}`;
    if (p.style) prompt += `
脚本风格：${p.style}`;
    if (p.keywords) prompt += `
关键词标签：${p.keywords} (请在脚本中自然融入这些关键词)`;
    if (p.specialRequirements) prompt += `
特殊要求：${p.specialRequirements}`;
  }

  prompt += `

【创作要求】
1. 根据个性化信息调整脚本风格和表达方式
2. 确保内容符合目标受众的喜好
3. 融入指定的关键词和商业目的
4. 每个场景包含：scene(场景名称)、time(时间段)、visual(画面描述)、camera(镜头运动)、dialogue(对话)
5. 直接返回JSON数组格式，不要额外说明
    ...
    const response = await fetch('https://dashscope.aliyuncs.com/compatible-mode/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { 
            role: 'system', 
            content: '你是专业的视频分镜脚本创作者，擅长根据不同行业、风格和目标受众创作个性化脚本。你能够准确理解商业目的，并将个性化元素自然融入创作中，让脚本更具针对性和感染力。' 
          },
          { role: 'user', content: prompt }
        ],
        temperature: 0.8,
